    ...
    <div class="table">
        ...
        <div class="container">
            ...
            <div class="handle-box">
                ...
                <el-input v-model="select_word" placeholder="按名称搜索" class="handle-input mr10"></el-input>
                <el-button type="primary" icon="search" @click="search">搜索</el-button>
            </div>
            <!-- 表格 -->
            <el-table :data="tableData" border style="width: 100%" ref="multipleTable" @selection-change="handleSelectionChange">
                <el-table-column type="selection" width="55"></el-table-column>
                <el-table-column prop="topicID" label="话题ID">
                </el-table-column>
                <el-table-column prop="topicName" label="话题名">
                </el-table-column>
                <el-table-column prop="topicNum" label="评论人数">
                </el-table-column>
                <el-table-column prop="topicTime" label="发布时间">
                </el-table-column>
                <el-table-column prop="topicContent" label="主题内容">
                </el-table-column>
        ...
        <el-dialog title="提示" :visible.sync="delVisible" width="300px" center>
            <div class="del-dialog-cnt">删除话题不可恢复，是否确定删除？</div>
            <span slot="footer" class="dialog-footer">
        ...
        <el-dialog title="提示" :visible.sync="delAllVisible" width="300px" center>
            <div class="del-dialog-cnt">批量删除话题不可恢复，是否确定删除？</div>
            <span slot="footer" class="dialog-footer">
...
import qs from 'qs';
    export default {
        ...
        methods: {
            ...
            getData() {
                var that = this;
                this.$axios.post('http://39.107.66.152:8080/admin/topic',qs.stringify({page_size:7,cur_page:that.cur_page})).then((res) => {
            ...
            search() {
                if(!this.select_word){
                    ...
                    this.seen = false;
                    var that = this;
                    that.tableData = that.filterForm('topicName','topicContent');
                }
            },
            ...
            filterForm(word1,word2){
                return this.tableData.filter((d) => {
                            ...
                            if (!is_del) {
                                // console.log(d)
                                if (d[word1].indexOf(this.select_word) > -1 || d[word1].indexOf(this.select_word) > -1) {
                                    console.log(d)
                                    return d;
                                }
                            }
            ...
            delAll() {
                ...
                for(var i=0;i<this.multipleSelection.length;i++){
                    delArr.push(this.multipleSelection[i].topicID);
                }
                this.delForm(delArr);
            ...
            delForm(arr){
                if(arr == []){
                   this.$message.error(`请先选择活动！`); 
                    ...
                    var that = this;
                    this.$axios.post('http://39.107.66.152:8080/admin/rmTopic',
                        qs.stringify({
                           delArr:arr
                            })).then((res) => {
                                if(res.data == '1'){
            ...
            deleteRow(){
                let delArr = []
                delArr.push(this.tableData[this.idx].topicID);
                this.delForm(delArr);
